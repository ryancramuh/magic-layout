...
PORT ?= 2020
GAWRC := $(HOME)/.gaw/gawrc

.PHONY: fixgaw
fixgaw:
	@if [ ! -f $(GAWRC) ]; then \
		echo "Launching gaw once to create $(GAWRC)..."; \
		gaw -v >/dev/null 2>&1 || true; \
	fi
	@if grep -q "^up_listenPort" $(GAWRC); then \
		sed -i 's|^up_listenPort = .*|up_listenPort = $(PORT)|' $(GAWRC); \
	else \
		echo "up_listenPort = $(PORT)" >> $(GAWRC); \
	fi
